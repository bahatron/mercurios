...
docker exec mercurios-postgres sh -c "wait-for-it localhost:4254 -t 30 -- npm run test"
docker exec mercurios-mysql sh -c "wait-for-it localhost:4254 -t 30 -- npm run test"

MERCURIOS_URL=http://mercurios-postgres:4254 docker exec mercurios-client sh -c "npm run test"
